...
def protected():
    ...
    profile_picture_url = f"http://194.31.53.102/upload_profile/{user.profile_picture}"
    return jsonify(
        name=user.name,
        email=user.email,
...
#     cap = cv2.VideoCapture(0)  # Connect to the smartphone camera stream
...
def request_reset_password():
    ...
    token = token_hex(3)  # Generates a shorter 32-character token
    reset_url = url_for('reset_password', token=token, _external=True)

    msg = Message('Password Reset Request', sender='aldi.bhaz10@gmail.com', recipients=[email])
    msg.body = f'Click the link to reset your password: {reset_url}'
    mail.send(msg)

    user_collection.update_one({'email': email}, {'$set': {'reset_token': token, 'token_created_at': datetime.datetime.utcnow()}})

    return jsonify({'message': 'Password reset email sent'}), 200

@app.route('/reset_password/<token>', methods=['GET', 'POST'])
def reset_password(token):
    user_data = user_collection.find_one({'reset_token': token})

    if not user_data:
        return jsonify({'message': 'The token is either invalid or has expired'}), 400

    token_age = (datetime.datetime.utcnow() - user_data['token_created_at']).total_seconds()
    ...
    user = user_collection.find_one({'email': user_data['email'], 'otp': code})
    if user:
        hashed_password = generate_password_hash(new_password, method='pbkdf2:sha256')
        user_collection.update_one({'_id': user['_id']}, {'$set': {'password': hashed_password, 'otp': None, 'reset_token': None, 'token_created_at': None}})
...
if __name__ == '__main__':
    app.run(debug=True, host='192.168.51.112')